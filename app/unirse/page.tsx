...
export default function Unirse() {
  ...
  const buscarGrupoConCodigo = async (cod: string) => {
    ...
    setBuscando(true);
    try {
      // Buscar en grupos de ligas
      const q1 = query(collection(db, 'grupos'), where('codigo', '==', cod));
      const snap1 = await getDocs(q1);
      if (!snap1.empty) {
        const data = { id: snap1.docs[0].id, ...snap1.docs[0].data() } as any;
        setGrupo(data);
        setEsMundial(false);
        if (data.precio) setShowAlerta(true);
        setBuscando(false);
        return;
      }

      // Buscar en grupos del Mundial
      const q2 = query(collection(db, 'grupos_mundial'), where('codigo', '==', cod));
      const snap2 = await getDocs(q2);
      if (!snap2.empty) {
        const data = { id: snap2.docs[0].id, ...snap2.docs[0].data() } as any;
        setGrupo(data);
        setEsMundial(true);
        if (data.precio) setShowAlerta(true);
        setBuscando(false);
  ...
  const unirse = async () => {
    ...
    setUniendose(true);
    try {
      const coleccion = esMundial ? 'grupos_mundial' : 'grupos';
      await updateDoc(doc(db, coleccion, grupo.id), { miembros: arrayUnion(user.uid) });
      router.push(esMundial ? `/mundial/grupo/${grupo.id}` : `/grupo/${grupo.id}`);
    } catch (e) {
  ...
  return (
    <main className="min-h-screen bg-[#020810] max-w-md mx-auto pb-20">
      ...
      <div style={{ background: esMundial ? 'linear-gradient(160deg,#0d0d1a,#1a1a2e)' : 'linear-gradient(160deg,#0A1F5C,#0D2870)' }} className="px-4 pt-4 pb-5">
      ...
      <div className="px-4 py-4">
        ...
        {grupo && (
          <div className="rounded-2xl overflow-hidden mb-4"
            style={{ background: '#0D1B3E', border: `1px solid ${esMundial ? 'rgba(200,170,110,0.25)' : 'rgba(0,200,83,0.25)'}` }}>
            <div className="p-4">
              <div className="flex items-center gap-2 mb-3">
                <div className="text-2xl">{esMundial ? '🏆' : '👥'}</div>
                <div>
                  <div className="font-condensed text-lg font-black" style={{ color: esMundial ? '#C8AA6E' : '#F5F5F0' }}>{grupo.nombre}</div>
                  <div className="text-xs" style={{ color: '#8892A4' }}>
                    {esMundial ? 'Mundial 2026 · Fase de Grupos' : (LIGAS[grupo.liga] || grupo.liga)}
                  </div>
                </div>
              </div>
              {grupo.precio && (
              ...
              <button onClick={() => grupo.precio ? setShowAlerta(true) : unirse()} disabled={uniendose}
                ...
                style={{ background: esMundial ? '#C8AA6E' : '#00C853', color: esMundial ? '#0d0d1a' : 'white', opacity: uniendose ? 0.7 : 1 }}>
