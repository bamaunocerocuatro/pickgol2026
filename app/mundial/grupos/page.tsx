...
export default function GruposMundial() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      setCargando(true);
      try {
        const q = query(collection(db, 'grupos_mundial'), where('miembros', 'array-contains', u.uid));
    ...
    <main className="min-h-screen bg-[#020810] max-w-md mx-auto pb-20">
      ...
      <div className="px-4 py-4">
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={() => router.push('/unirse?mundial=1')}
        ...
        {!cargando && grupos.length === 0 && (
          <div className="rounded-2xl mb-4 overflow-hidden" style={{ background: '#0D1B3E', border: '1px solid rgba(200,170,110,0.15)' }}>
        ...
        {!cargando && grupos.map((g) => (
          <div key={g.id} onClick={() => router.push(`/mundial/grupo/${g.id}`)}
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              {g.creadorId === user?.uid && (
                <span className="text-xs px-2 py-1 rounded-lg font-bold" style={{ background: 'rgba(200,170,110,0.12)', color: '#C8AA6E' }}>👑 Creador</span>
              )}
            </div>
